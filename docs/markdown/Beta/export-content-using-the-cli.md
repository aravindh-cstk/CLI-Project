...
# Export Content Using the CLI | V2.x.x Beta
...
| `--branch=branch` | - | Name of the branch from which content is exported.<br>Default: `all branches`<br>If not specified, content from all branches is exported. |
| `--branch-alias=branch-alias` |  | Alias of the branch from which content is exported. |
| `--module=module` | - | \[Optional\] Export a specific module from the source stack.<br>If not specified, all modules are exported.<br>Available modules: `assets`, `content-types`, `entries`, `environments`, `stacks`, `extensions`, `marketplace-apps`, `global-fields`, `labels`, `locales`, `webhooks`, `workflows`, `custom-roles`, `taxonomies`, `personalize`, `studio` |
