...
# Migrate your Content using the CLI Migration Command | V2.x.x Beta
...
| --- | --- | --- |
| `--stack-api-key` | `-k` | Use this flag to add the API key of your stack.<br>You must use either the `--stack-api-key` flag or the `--alias` flag. |
| `--alias` | `-a` | Use this flag to add the management token alias.<br>You must use either the `--alias` flag or the `--stack-api-key` flag. |
